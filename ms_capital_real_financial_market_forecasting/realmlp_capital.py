#!/usr/bin/env python3
"""
Converted from rfmf-realmlp.ipynb
MSCapital - Real Financial Market Forecasting: RealMLP-style PyTorch model
with a 16-member parallel ensemble, periodic numerical embeddings, and
residual-quantized (RQ-KMeans) auxiliary target encoding.
"""

# ===================================================================
# Cell 1: Setup, seeds, device, hyperparameters
# ===================================================================
import os
import random
import numpy as np
...
n_ens = 16
embed_dim = 6
LR = 0.003459544068412455  # learning rate
epochs = 10
train_bs = 256   # batch size
eval_bs = train_bs
target_col = 'y'
...
# ===================================================================
# Cell 2: Data loading, correlation-based feature selection, quantile binning
# ===================================================================
...
# every numerical column with >100 unique values gets discretized into 40 quantile bins
...
# ===================================================================
# Cell 4: RobustScaleSmoothClipTransform (numerical preprocessing)
# ===================================================================
# 预处理类
from sklearn.base import BaseEstimator, TransformerMixin

class RobustScaleSmoothClipTransform(BaseEstimator, TransformerMixin):
    # centers by median, scales by IQR (75th–25th percentile) instead of mean/std
...
# ===================================================================
# Cell 5: Core model building blocks (ScalingLayer, CategoricalFeatureLayer, PBLDEmbedding, NTPLinear)
# ===================================================================
class ScalingLayer(nn.Module):
    # a learnable per-feature, per-ensemble-member scale (like a diagonal weight matrix) 
    # applied right after input normalization — a RealMLP-specific trick.
...
class CategoricalFeatureLayer(nn.Module):
    # parallel-ensemble structure for categorical features — 
    # low-cardinality ones get one-hot encoded per ensemble member, 
    # high-cardinality ones get embedded via one shared embedding table indexed with per-ensemble-member offsets 
    # (a clever vectorization: instead of 16 separate embedding tables, it uses one big table 
    # and computes indices that route each ensemble member to its own disjoint slice).
...
# ===================================================================
# Cell 6: Training utilities (LR schedules, EMA)
# ===================================================================
import math
...
# ===================================================================
# Cell 7: RQKMeansEncoder + RealMLP_RQ model definition
# ===================================================================
...
# ===================================================================
# Cell 8: Data prep, loss function, training loop (train/val split)
# ===================================================================
# ============ 准备数据 ============
...
# param_2
def compute_loss_with_rq(y_pred, y_true, code_logits, y_codes, lambda_cos=0.037536958059937314, lambda_rq=0.3115719017291383):
    if y_pred.dim() == 3:
...
optimizer = torch.optim.AdamW([
    {'params': scale_p,        'lr': LR * 12.440877505762993,  'weight_decay': 0.0016556093611899566 * 0.1},  # lr_scale_mult
    {'params': pbld_p,         'lr': LR * 0.20550183452486626, 'weight_decay': 0.0016556093611899566},   # lr_pbld_mult
    {'params': first_linear_p, 'lr': LR * 1.0938977642558907,   'weight_decay': 0.0016556093611899566 * 0.1},   # lr_first_mult
    {'params': other_w_p,      'lr': LR,         'weight_decay': 0.0016556093611899566},
    {'params': bias_p,         'lr': LR * 0.09616034562787786,   'weight_decay': 0.0016556093611899566 * 0.5},   # lr_bias_mult
], betas=(0.9, 0.98))

# param_4
# ============ EMA ============
use_ema = True
ema_decay = 0.9978462575717068
...
for epoch in range(epochs):
    ...
    for i in range(0, len(y_train_tensor), train_bs):
		...
		# param_5
        # ============ 动态更新学习率 ============
        optimizer.param_groups[0]['lr'] = flat_anneal(LR * 12.440877505762993, progress)   # lr_scale_mult
        optimizer.param_groups[1]['lr'] = flat_anneal(LR * 0.20550183452486626, progress)    # lr_pbld_mult
        optimizer.param_groups[2]['lr'] = flat_anneal(LR * 1.0938977642558907, progress) # lr_first_mult
        optimizer.param_groups[3]['lr'] = flat_anneal(LR, progress)
        optimizer.param_groups[4]['lr'] = flat_anneal(LR * 0.09616034562787786, progress) # lr_bias_mult
        ...
        loss, cos_sim, mse_loss, rq_loss = compute_loss_with_rq(
            ...
            lambda_cos=0.037536958059937314, lambda_rq=flat_anneal(0.3115719017291383, progress)  # lambda_cos lambda_rq
        )
...
# ===================================================================
# Cell 9: Full-data retrain (train + val combined)
# ===================================================================
# ============ 7. 用全量数据重新训练（参数完全一致） ============
print("\n使用全量数据重新训练...")
...
optimizer_final = torch.optim.AdamW([
    {'params': scale_p,        'lr': LR * 12.440877505762993,  'weight_decay': 0.0016556093611899566 * 0.1},
    {'params': pbld_p,         'lr': LR * 0.20550183452486626, 'weight_decay': 0.0016556093611899566},
    {'params': first_linear_p, 'lr': LR * 1.0938977642558907,   'weight_decay': 0.0016556093611899566 * 0.1},
    {'params': other_w_p,      'lr': LR,         'weight_decay': 0.0016556093611899566},
    {'params': bias_p,         'lr': LR * 0.09616034562787786,   'weight_decay': 0.0016556093611899566 * 0.5},
], betas=(0.9, 0.98))
...
for epoch in range(epochs):
    ...
    for i in range(0, len(y_full_tensor), train_bs):
		...
		# param_8
        # ============ 动态更新学习率 ============
        optimizer_final.param_groups[0]['lr'] = flat_anneal(LR * 12.440877505762993, progress)
        optimizer_final.param_groups[1]['lr'] = flat_anneal(LR * 0.20550183452486626, progress)
        optimizer_final.param_groups[2]['lr'] = flat_anneal(LR * 1.0938977642558907, progress)
        optimizer_final.param_groups[3]['lr'] = flat_anneal(LR, progress)
        optimizer_final.param_groups[4]['lr'] = flat_anneal(LR * 0.09616034562787786, progress)
        ...
        loss, cos_sim, mse_loss, rq_loss = compute_loss_with_rq(
            y_pred, batch_y_noisy, code_logits, batch_y_rq,
            lambda_cos=0.037536958059937314, lambda_rq=0.3115719017291383
        )
...
# ===================================================================
# Cell 10: Inference on test set + submission
# ===================================================================
# ============ 8. 推理 ============
print("\n开始推理测试集...")
...
sample_submission.to_csv('realmlp_submission_146082.csv', index=False)
